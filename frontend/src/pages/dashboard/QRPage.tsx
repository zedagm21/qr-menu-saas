...
const COLOR_PRESETS = [
    ...
    { id: 'amber', name: 'Terracotta Amber', fg: '#ea580c', bg: '#ffffff', accent: '#c2410c' },
    ...
    { id: 'violet', name: 'Plum Violet', fg: '#7c3aed', bg: '#ffffff', accent: '#6d28d9' },
];
...
export default function QRPage() {
    ...
    // Ref for offscreen high-res rendering
    ...
    const handleDownloadPNG = async () => {
        ...
        try {
            ...
            // Capture the styled designer card in high resolution (3x scale)
            const cardEl = (downloadCanvasRef.current.firstElementChild as HTMLElement) || downloadCanvasRef.current;
    ...
    return (
        <>
            ...
            {/* Offscreen full-resolution card for crisp html2canvas PNG export */}
            <div
                style={{
                    position: 'fixed',
                    left: -9999,
            ...
            <div className="min-h-full bg-gradient-to-br from-neutral-50 via-white to-neutral-100/80 dark:from-neutral-950 dark:via-neutral-900/90 dark:to-neutral-900 px-3.5 py-4 sm:p-6 lg:p-8 pb-28 lg:pb-12 transition-colors duration-200 print:hidden">
                <div className="max-w-2xl lg:max-w-6xl mx-auto space-y-4 sm:space-y-6 lg:space-y-8">

                    {/* ── Page Header ── */}
                    <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                        <div className="space-y-1">
                            <div className="flex items-center gap-2">
                                <span className="px-2 py-0.5 sm:px-2.5 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-wider bg-[color:var(--color-brand-50)] dark:bg-[color:var(--color-brand-500)]/10 text-[color:var(--color-brand-700)] dark:text-[color:var(--color-brand-400)] border border-[color:var(--color-brand-200)] dark:border-[color:var(--color-brand-500)]/20 flex items-center gap-1">
                                    <Sparkles className="w-3 h-3" />
                                    <span>{t('qr.studio_badge', { defaultValue: 'Custom QR Studio' })}</span>
                                </span>
                            </div>
                            <h1 className="text-xl sm:text-3xl lg:text-4xl font-extrabold text-neutral-900 dark:text-neutral-50 tracking-tight">
                        ...
                        {/* Top quick actions (Desktop only: hidden on mobile, visible on lg screens) */}
                        <div className="hidden lg:flex items-center gap-2.5 flex-wrap">
                            <Button
                                variant="outline"
                                onClick={handleShare}
                                disabled={!currentSlug}
                                className="h-10 px-4 rounded-xl border-neutral-300 dark:border-neutral-700 text-neutral-700 dark:text-neutral-300 font-bold cursor-pointer"
                    ...
                    {!currentSlug && (
                        <div className="p-4 sm:p-6 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-950 dark:text-amber-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 animate-fade-in">
                            ...
                            <Link to="/dashboard/restaurant">
                                <Button variant="primary" size="sm" className="shrink-0 font-bold text-xs">
                    ...
                    {isLoading ? (
                        ...
                        <div className="backdrop-blur-md bg-white/95 dark:bg-neutral-900/95 border border-neutral-200/90 dark:border-neutral-800/90 rounded-2xl p-8 sm:p-10 text-center shadow-xs">
                            <div className="w-16 h-16 mx-auto rounded-2xl bg-[color:var(--color-brand-50)] dark:bg-[color:var(--color-brand-500)]/10 text-[color:var(--color-brand-600)] dark:text-[color:var(--color-brand-400)] flex items-center justify-center text-3xl mb-4 shadow-xs">
                                🪄
                            </div>
                            <h2 className="text-xl font-black text-neutral-900 dark:text-neutral-50 mb-2">
                                {t('qr.generate_title', { defaultValue: 'Generate Your QR Code' })}
                            </h2>
                            <p className="text-xs sm:text-sm text-neutral-500 dark:text-neutral-400 mb-6 max-w-sm mx-auto leading-relaxed">
                            ...
                            <Button
                                variant="primary"
                                className="h-11 px-6 rounded-xl bg-[color:var(--color-brand-500)] hover:bg-[color:var(--color-brand-600)] text-white font-bold shadow-md"
                                onClick={() => ensure()}
                        ...
                        <>
                            {/* ════════════════════════════════════════════════════════════════════════
                                DESKTOP STUDIO LAYOUT (Visible on lg screens: 1024px and up)
                                Preserves the wide 2-column studio with sticky 3D preview and tabs
                            ════════════════════════════════════════════════════════════════════════ */}
                            <div className="hidden lg:grid lg:grid-cols-12 gap-8 items-start animate-fade-in">

                                {/* ── Left Column: Sticky Live 3D Card Preview ── */}
                                <div className="lg:col-span-5 space-y-5 lg:sticky lg:top-20">
                                    <div className="relative rounded-3xl p-6 sm:p-8 bg-gradient-to-br from-neutral-100 via-neutral-50 to-neutral-200 dark:from-neutral-900 dark:via-neutral-950 dark:to-neutral-900 border border-neutral-200/80 dark:border-neutral-800 flex flex-col items-center justify-center min-h-[480px] shadow-inner overflow-hidden">
                                        ...
                                        <p className="text-[11px] font-semibold text-neutral-400 dark:text-neutral-500 mt-4 flex items-center gap-1.5">
                                    ...
                                    <div className="backdrop-blur-md bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-2 flex items-center gap-2 shadow-xs">
                                        ...
                                        <button
                                            type="button"
                                            onClick={handleCopy}
                                            className="h-9 px-3 rounded-xl bg-neutral-100 hover:bg-neutral-200 dark:bg-neutral-800 dark:hover:bg-neutral-700 text-neutral-700 dark:text-neutral-200 text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer shrink-0"
                                        >
                                    ...
                                    {activeTab === 'color' && (
                                        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6 space-y-6 animate-fade-in shadow-xs">
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-2 gap-3">
                                                {COLOR_PRESETS.map((p) => {
                                                    const isSelected = config.fgColor.toLowerCase() === p.fg.toLowerCase();
                                                    return (
                                                        <button
                                                            key={p.id}
                                                            type="button"
                                                        ...
                                                        >
                                                            <div
                                                                className="w-7 h-7 rounded-xl shadow-xs border border-black/10 shrink-0 flex items-center justify-center"
                                        ...
                                        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6 space-y-5 animate-fade-in shadow-xs">
                                            ...
                                            <div className="p-4 rounded-2xl bg-neutral-50 dark:bg-neutral-800/40 border border-neutral-200 dark:border-neutral-700 flex items-center justify-between gap-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-10 h-10 rounded-xl bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 flex items-center justify-center text-lg overflow-hidden shrink-0">
                                                        {restaurant?.logoUrl ? (
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {LANGUAGE_PRESETS.map((preset) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                {isSelected && <Check className="w-3.5 h-3.5 stroke-[3] text-emerald-400 dark:text-emerald-600 shrink-0" />}
                            ...
                            {/* ════════════════════════════════════════════════════════════════════════
                                MOBILE SINGLE-PAGE FLOW (Visible below 1024px: lg:hidden)
                                Preserves the compact, native single-page layout tailored for phones
                            ════════════════════════════════════════════════════════════════════════ */}
                            <div className="lg:hidden space-y-4 sm:space-y-5 animate-fade-in">

                            {/* ── Section 1: Wider Preview & 2-Column Palette (Side-by-Side) ── */}
                            <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl sm:rounded-3xl p-3.5 sm:p-5 shadow-xs">
                                <div className="flex flex-row items-center gap-3.5 sm:gap-6">

                                    {/* Left: Wider Card Preview */}
                                    <div className="flex-1 min-w-0 flex items-center justify-center">
                                        <div className="relative w-full rounded-2xl p-2.5 sm:p-4 bg-gradient-to-br from-neutral-100 via-neutral-50 to-neutral-200 dark:from-neutral-900 dark:via-neutral-950 dark:to-neutral-900 border border-neutral-200/80 dark:border-neutral-800 flex items-center justify-center overflow-hidden shadow-inner max-w-[230px] sm:max-w-[270px]">
                                            {/* Ambient backdrop glow */}
                                            <div
                                    ...
                                    {/* Right: Palette (2-column, compact to save space) */}
                                    <div className="shrink-0 w-[84px] sm:w-[96px] space-y-2 sm:space-y-2.5">
                                        <div>
                                            <h3 className="text-xs sm:text-sm font-extrabold text-neutral-900 dark:text-white flex items-center gap-1">
                                                <Palette className="w-3.5 h-3.5 text-[color:var(--color-brand-500)] shrink-0" />
                                                <span>Palette</span>
                                            </h3>
                                        </div>

                                        {/* Click and switch color swatches - 2 columns, no names */}
                                            ...
                                            {COLOR_PRESETS.map((p) => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        onClick={() => setConfig(c => ({ ...c, fgColor: p.fg }))}
                                                        className={cn(
                                                            'w-8 h-8 sm:w-9 sm:h-9 rounded-xl transition-all cursor-pointer flex items-center justify-center relative shadow-xs border',
                                                            isSelected
                                                                ? 'ring-2 ring-offset-2 ring-[color:var(--color-brand-500)] scale-105 border-white dark:border-neutral-900'
                                        ...
                                        <div className="flex items-center gap-1.5 pt-1.5 border-t border-neutral-100 dark:border-neutral-800">
                                            <label className="relative cursor-pointer shrink-0">
                                                ...
                                                />
                                                <div
                                                    className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg shadow-xs border border-neutral-300 dark:border-neutral-700 flex items-center justify-center cursor-pointer transition-transform hover:scale-105"
                                                    style={{ backgroundColor: config.fgColor }}
                                                    title="Custom Color"
                                                >
                                                    <Palette className="w-3.5 h-3.5 text-white drop-shadow-xs opacity-90" />
                                                </div>
                                            </label>
                                            <input
                                                type="text"
                                                value={config.fgColor}
                                                onChange={e => setConfig(c => ({ ...c, fgColor: e.target.value }))}
                                                className="w-full h-7 sm:h-8 px-1 rounded-lg border border-neutral-200 dark:border-neutral-700 text-[10px] font-mono font-bold uppercase bg-neutral-50 dark:bg-neutral-800 text-neutral-800 dark:text-neutral-200 text-center focus:outline-none focus:ring-1 focus:ring-[color:var(--color-brand-500)]"
                                                placeholder="#000"
                                            />
                                        </div>
                                    </div>
                            ...
                            {/* ── Section 2: Stand Style (Two Rows, Each Holding Two) ── */}
                            <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl sm:rounded-3xl p-3.5 sm:p-5 shadow-xs space-y-3">
                                <div>
                                    <h3 className="text-xs sm:text-sm font-extrabold text-neutral-900 dark:text-white">
                                        {t('qr.choose_template', { defaultValue: 'Stand Style' })}
                                    </h3>
                                    <p className="text-[10px] sm:text-[11px] text-neutral-500 dark:text-neutral-400 mt-0.5">
                                        {t('qr.choose_template_desc', { defaultValue: 'Choose a card layout for your tables' })}
                                    </p>
                                </div>
                                    ...
                                    {TEMPLATES.map((tmpl) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                onClick={() => setConfig(c => ({ ...c, template: tmpl.id }))}
                                                className={cn(
                                                    'p-2.5 sm:p-3 rounded-xl sm:rounded-2xl border-2 text-left transition-all cursor-pointer flex items-center gap-2 sm:gap-3',
                                                    isSelected
                                                        ? 'border-[color:var(--color-brand-500)] bg-[color:var(--color-brand-50)]/50 dark:bg-[color:var(--color-brand-500)]/10 shadow-xs'
                                                        : 'border-neutral-200 dark:border-neutral-800 hover:border-neutral-300 dark:hover:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-800/30'
                                                    )}
                                            >
                                                <span className="text-lg sm:text-xl p-1.5 rounded-xl bg-white dark:bg-neutral-800 shadow-xs border border-neutral-200 dark:border-neutral-700 shrink-0">
                                                    {tmpl.icon}
                                                </span>
                            ...
                            {/* ── Section 3: Text & Branding ── */}
                            <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl sm:rounded-3xl p-3.5 sm:p-5 shadow-xs space-y-4">
                                <div>
                                    <h3 className="text-xs sm:text-sm font-extrabold text-neutral-900 dark:text-white">
                                        {t('qr.branding_title', { defaultValue: 'Text & Branding' })}
                                    </h3>
                                    <p className="text-[10px] sm:text-[11px] text-neutral-500 dark:text-neutral-400 mt-0.5">
                                        {t('qr.branding_desc', { defaultValue: 'Personalize prompt, language, and center badge' })}
                                    </p>
                                </div>

                                {/* Center Logo Toggle */}
                                <div className="p-3 rounded-xl sm:rounded-2xl bg-neutral-50 dark:bg-neutral-800/40 border border-neutral-200 dark:border-neutral-700 flex items-center justify-between gap-3">
                                    <div className="flex items-center gap-2.5 min-w-0">
                                        <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 flex items-center justify-center text-sm overflow-hidden shrink-0">
                                            {restaurant?.logoUrl ? (
                                                <img src={resolveImageUrl(restaurant.logoUrl) || restaurant.logoUrl} alt="Logo" className="w-full h-full object-cover" />
                                            ) : (
                                                <span className="text-xs font-black text-neutral-800 dark:text-neutral-200">
                                        ...
                                        <div className="min-w-0">
                                            <h4 className="text-xs sm:text-sm font-extrabold text-neutral-900 dark:text-white truncate">
                                                {t('qr.center_logo', { defaultValue: 'Embed Restaurant Logo' })}
                                            </h4>
                                            <p className="text-[10px] sm:text-[11px] text-neutral-500 dark:text-neutral-400 truncate">
                                                {restaurant?.logoUrl
                                                    ? 'Places logo in center with high error correction'
                                                    : 'Uses restaurant logo or brand initial'}
                                            </p>
                                        </div>
                                    </div>
                                    <button
                                        ...
                                        onClick={() => setConfig(c => ({ ...c, includeLogo: !c.includeLogo }))}
                                        className={cn(
                                            'relative inline-flex h-5 w-9 sm:h-6 sm:w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200',
                                            config.includeLogo ? 'bg-[color:var(--color-brand-500)]' : 'bg-neutral-300 dark:bg-neutral-700'
                                        )}
                                    >
                                        <span
                                            className={cn(
                                                'pointer-events-none inline-block h-4 w-4 sm:h-5 sm:w-5 transform rounded-full bg-white shadow-md transition duration-200',
                                                config.includeLogo ? 'translate-x-4 sm:translate-x-5' : 'translate-x-0'
                                            )}
                                        />
                                    </button>
                                </div>
                                        ...
                                        {LANGUAGE_PRESETS.map((preset) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={cn(
                                                        'px-2.5 py-1.5 rounded-xl text-[11px] sm:text-xs font-bold transition-all cursor-pointer border flex items-center gap-1.5',
                                                        isSelected
                                                            ? 'bg-neutral-900 text-white dark:bg-white dark:text-neutral-950 border-neutral-900 dark:border-white shadow-xs'
                                                            : 'bg-neutral-100 hover:bg-neutral-200/80 dark:bg-neutral-800 dark:hover:bg-neutral-700 text-neutral-700 dark:text-neutral-300 border-neutral-200 dark:border-neutral-700'
                                                    )}
                                                >
                                                    {isSelected && <Check className="w-3 h-3 stroke-[3] text-emerald-400 dark:text-emerald-600 shrink-0" />}
                                                    <span>{preset.label}</span>
                                                </button>
                                            );
                                        })}
                                ...
                                {/* CTA Text Input */}
                                <div>
                                    <label className="text-xs font-extrabold text-neutral-700 dark:text-neutral-300 block mb-1">
                                        {t('qr.cta_label', { defaultValue: 'Call to Action Prompt' })}
                                    </label>
                                    <input
                                        type="text"
                                        ...
                                        onChange={e => setConfig(c => ({ ...c, ctaText: e.target.value }))}
                                        placeholder="e.g. Scan to View Menu • የምግብ ዝርዝር ለማየት በስልክዎ ስካን ያድርጉ"
                                        className="w-full h-10 px-3.5 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-800 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-[color:var(--color-brand-500)]/30"
                                    />
                                </div>
                                ...
                                <div className="pt-2.5 border-t border-neutral-100 dark:border-neutral-800 space-y-2">
                                        ...
                                        >
                                            <span
                                                className={cn(
                                                    ...
                                                    config.showSubtitle ? 'translate-x-4' : 'translate-x-0'
                                                )}
                                            />
                                        </button>
                                    </div>
                                    {config.showSubtitle && (
                                        <input
                                            ...
                                            onChange={e => setConfig(c => ({ ...c, subText: e.target.value }))}
                                            placeholder="e.g. Contactless Digital Menu • ዲጂታል የምግብ ዝርዝር"
                                            className="w-full h-9 px-3 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-800 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-[color:var(--color-brand-500)]/30"
                                        />
                                    )}
                                </div>
                                ...
                                <div className="pt-2.5 border-t border-neutral-100 dark:border-neutral-800 space-y-2">
                                    <div className="flex items-center justify-between">
                                        ...
                                        >
                                            <span
                                                className={cn(
                                                    'pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-md transition duration-200',
                                                    config.showWifi ? 'translate-x-4' : 'translate-x-0'
                                                )}
                                            />
                                        </button>
                                    </div>
                                    {config.showWifi && (
                                        <input
                                            ...
                                            onChange={e => setConfig(c => ({ ...c, wifiName: e.target.value }))}
                                            placeholder="e.g. BlueNile_Guest (Pass: 12345678)"
                                            className="w-full h-9 px-3 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-800 text-xs font-medium"
                                        />
                                    )}
                                </div>
                            </div>

                            {/* ── Section 4: Export & Print Studio ── */}
                            <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl sm:rounded-3xl p-3.5 sm:p-5 shadow-xs space-y-3.5">
                                <div>
                                    <h3 className="text-xs sm:text-sm font-extrabold text-neutral-900 dark:text-white">
                                        {t('qr.export_center', { defaultValue: 'Export & Print' })}
                                    </h3>
                                    <p className="text-[10px] sm:text-[11px] text-neutral-500 dark:text-neutral-400 mt-0.5">
                                        Download high-resolution graphics or print ready-to-fold table stands
                                    </p>
                                </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                                    ...
                                    <Button
                                        ...
                                        onClick={handleDownloadPNG}
                                        isLoading={isDownloading}
                                        className="h-10 sm:h-11 rounded-xl bg-[color:var(--color-brand-500)] hover:bg-[color:var(--color-brand-600)] text-white font-extrabold text-xs shadow-xs cursor-pointer"
                                        icon={<Download className="w-4 h-4" />}
                                    >
                                        {t('qr.download_png', { defaultValue: 'Download Card PNG' })}
                                    </Button>
                                    ...
                                    <Button
                                        variant="outline"
                                        onClick={handleDownloadSVG}
                                        className="h-10 sm:h-11 rounded-xl border-neutral-300 dark:border-neutral-700 text-neutral-800 dark:text-neutral-200 font-extrabold text-xs hover:bg-neutral-50 dark:hover:bg-neutral-800 cursor-pointer"
                                        icon={<QrCode className="w-4 h-4" />}
                                    >
                                        {t('qr.download_svg', { defaultValue: 'Vector QR (SVG)' })}
                                    </Button>
                                ...
                                {/* URL Snippet */}
                                <div className="p-2 rounded-xl bg-neutral-50 dark:bg-neutral-800/60 border border-neutral-200/80 dark:border-neutral-700/80 flex items-center gap-2">
                                    <span className="text-[11px] sm:text-xs text-neutral-600 dark:text-neutral-300 flex-1 truncate font-mono px-2 select-all">
                                        {menuUrl}
                                    </span>
                                    <button
                                        type="button"
                                        onClick={handleCopy}
                                        className="h-7 sm:h-8 px-2.5 sm:px-3 rounded-lg bg-white hover:bg-neutral-100 dark:bg-neutral-800 dark:hover:bg-neutral-700 text-neutral-800 dark:text-neutral-200 text-[11px] sm:text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer shadow-xs border border-neutral-200 dark:border-neutral-700 shrink-0"
                                    >
                                        {copied ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}
                                        <span>{copied ? t('common.copied', { defaultValue: 'Copied' }) : t('common.copy', { defaultValue: 'Copy' })}</span>
                                    </button>
