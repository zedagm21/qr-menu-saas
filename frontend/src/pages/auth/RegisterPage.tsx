...
import { QrCode, Eye, EyeOff, Mail, Lock, User, Globe, Sun, Moon } from 'lucide-react';
...
const RegisterPage: React.FC = () => {
    ...
    return (
        <>
            <Helmet>
                <title>{t('auth.registerTitle')} — OurMenu</title>
            </Helmet>
                ...
                <main className="relative z-10 flex-1 flex items-center justify-center px-4 sm:px-6 py-6 sm:py-10">
                    <div className="w-full max-w-md">
                        ...
                        <p className="text-center mt-6 text-sm text-neutral-500 dark:text-neutral-400">
                            {t('auth.hasAccount')}{' '}
                            <Link to="/login" className="text-amber-600 dark:text-amber-500 font-semibold hover:text-amber-700 dark:hover:text-amber-400 transition-colors">
                                {t('auth.signInHere')}
                            </Link>
                        </p>
                    </div>
