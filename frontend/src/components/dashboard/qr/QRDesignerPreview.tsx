        ...
        return (
            <div
                className={cn(
                    ...
                    isCompact && 'w-full max-w-[230px] sm:max-w-[270px] rounded-2xl p-3 sm:p-4 pt-3.5 sm:pt-4 shadow-md border',
            ...
            >
                ...
                <div className="relative z-10 w-full flex flex-col items-center">
                    {(isFull || isPrint || isCompact) && hasCustomLogo && resolvedLogo && (
                        <div className={cn(
                            ...
                            isCompact ? 'w-7 h-7 rounded-lg mb-1 p-0.5' :
                    ...
                    <div
                        className={cn(
                            ...
                            isFull ? 'p-3.5 my-2.5 rounded-2xl' :
                            isCompact ? 'p-1.5 my-1 rounded-xl' : 'p-1 my-0.5'
                        )}
                    ...
                    <div
                        className={cn(
                            ...
                            isCompact ? 'mt-1.5 px-2 py-0.5 text-[7.5px] rounded-lg' :
                    ...
                    {config.showWifi && config.wifiName && (isFull || isPrint || isS1 || isS2 || isCompact) && (
                        <div className={cn(
                            ...
                            isP4 ? 'mt-3 text-sm' :
                            isP6 ? 'mt-2 text-xs' :
                            isFull ? 'mt-3 text-[11px]' :
                            isCompact ? 'mt-1 text-[7.5px]' : 'mt-1 text-[8px]'
                        )}>
                            <Wifi className={isP1 ? 'w-5 h-5' : isP2 ? 'w-4 h-4' : isCompact ? 'w-2.5 h-2.5' : 'w-3 h-3'} />
        ...
        return (
            <div
                className={cn(
                    ...
                    isCompact && 'w-full max-w-[230px] sm:max-w-[270px] rounded-2xl p-3 sm:p-4 pt-3.5 sm:pt-4 shadow-md border-2',
            ...
            >
                ...
                <div
                    className={cn(
                        ...
                        isP1 ? 'inset-6 rounded-3xl' : isP2 ? 'inset-5 rounded-2xl' : isP4 ? 'inset-3.5 rounded-xl' : isFull ? 'inset-3 rounded-2xl' : isCompact ? 'inset-1.5 rounded-xl' : 'inset-1'
                    )}
                    style={{ borderColor: config.fgColor }}
                />
                ...
                <div className="relative z-10 w-full flex flex-col items-center">
                    <div className={cn('flex items-center gap-1.5', isP1 ? 'mb-4' : isP2 ? 'mb-3' : isP4 ? 'mb-2' : isFull ? 'mb-2' : isCompact ? 'mb-0.5' : 'mb-0.5')}>
                        <Sparkles className={isP1 ? 'w-6 h-6' : isP2 ? 'w-5 h-5' : isP4 ? 'w-4 h-4' : isFull ? 'w-3.5 h-3.5' : 'w-2.5 h-2.5'} style={{ color: config.fgColor }} />
                        <span
                            className={cn(
                                'font-black uppercase tracking-widest',
                                isP1 ? 'text-lg' : isP2 ? 'text-base' : isP4 ? 'text-xs' : isFull ? 'text-[10px]' : isCompact ? 'text-[7.5px]' : 'text-[6px]'
                            )}
                            style={{ color: config.fgColor }}
                        >
                            Exquisite Dining
                    ...
                    <div
                        className={cn(
                            ...
                            isP4 ? 'p-4 my-2.5 rounded-2xl border-2' :
                            isP6 ? 'p-3 my-2 rounded-xl border' :
                            isFull ? 'p-3.5 my-2 rounded-2xl border-2' :
                            isCompact ? 'p-1.5 my-1 rounded-xl border' : 'p-1 my-0.5'
                        )}
                        style={{ borderColor: config.fgColor }}
                    >
                        <QRComponent
                    ...
                    <div className={cn('text-center leading-snug w-full px-1', isP1 ? 'mt-8' : isP2 ? 'mt-6' : isP4 ? 'mt-4' : isFull ? 'mt-4' : isCompact ? 'mt-1.5' : 'mt-1')}>
                        <p className={cn(
                            'font-bold tracking-wider uppercase text-white',
                            isP1 ? 'text-2xl' : isP2 ? 'text-xl' : isP4 ? 'text-base' : isFull ? 'text-xs' : isCompact ? 'text-[7.5px]' : isS1 ? 'text-[9px]' : 'text-[6.5px]'
                        )}>
        ...
        return (
            <div
                className={cn(
                    ...
                    isCompact && 'w-full max-w-[230px] sm:max-w-[270px] rounded-2xl shadow-md border',
            ...
            >
                ...
                <div
                    className={cn(
                        ...
                        isP1 ? 'p-8' : isP2 ? 'p-6' : isP4 ? 'p-4' : isFull ? 'p-4' : isCompact ? 'p-2' : isS1 ? 'p-2.5' : 'p-1'
                    )}
                    style={{ backgroundColor: config.fgColor }}
                >
                    <Utensils className={isP1 ? 'w-8 h-8 mb-1.5 opacity-90' : isP2 ? 'w-6 h-6 mb-1 opacity-90' : isFull ? 'w-4 h-4 mb-0.5 opacity-90' : isCompact ? 'w-2.5 h-2.5 mb-0.5 opacity-90' : 'w-2 h-2 opacity-90'} />
                    <h2 className={cn(
                        'font-black tracking-tight break-words max-w-full leading-snug px-1',
                        isP1 ? 'text-4xl' : isP2 ? 'text-3xl' : isP4 ? 'text-2xl' : isFull ? 'text-base' : isCompact ? 'text-[11.5px]' : isS1 ? 'text-xs' : 'text-[8.5px]'
                    )}>
                        {config.restaurantName || 'Restaurant Name'}
                    </h2>
                </div>
                ...
                <div className={cn(
                    ...
                    isP1 ? 'p-10' : isP2 ? 'p-8' : isP4 ? 'p-6' : isFull ? 'p-5' : isCompact ? 'p-2.5' : isS1 ? 'p-3' : 'p-1.5'
                )}>
                    <div className={cn(
                    ...
                    <div className={cn('text-center w-full px-1', isP1 ? 'mt-8 space-y-2' : isP2 ? 'mt-6 space-y-1.5' : isP4 ? 'mt-4 space-y-1' : isFull ? 'mt-3 space-y-1' : isCompact ? 'mt-1.5 space-y-0.5' : 'mt-1')}>
                        <h3 className={cn(
                            'font-black text-neutral-900 leading-snug',
                            isP1 ? 'text-3xl' : isP2 ? 'text-2xl' : isP4 ? 'text-lg' : isFull ? 'text-xs' : isCompact ? 'text-[8px]' : isS1 ? 'text-[9px]' : 'text-[7px]'
                        )}>
                            {config.ctaText || 'Scan to View Menu • የምግብ ዝርዝር ለማየት በስልክዎ ስካን ያድርጉ'}
                        </h3>
                        {config.showSubtitle && config.subText && (
                            <p className={cn(
                                ...
                                isP1 ? 'text-xl' : isP2 ? 'text-base' : isP4 ? 'text-sm' : isFull ? 'text-xs' : isCompact ? 'text-[7px]' : 'text-[6.5px]'
                            )}>
                                {config.subText}
                            </p>
                        )}
    ...
    return (
        ...
        >
            <div className={cn(
                ...
                isFull ? 'p-3.5 mb-3 rounded-2xl' :
                isCompact ? 'p-1.5 mb-1.5 rounded-xl' : 'p-1 mb-1'
            )}>
                <QRComponent
                    value={menuUrl}
                    size={qrSize}
            ...
            <h3 className={cn(
                ...
                isCompact ? 'text-[12px] mb-0.5 font-extrabold' :
...
export const QRDesignerPreview: React.FC<QRDesignerPreviewProps> = ({
    ...
    return (
        <div ref={canvasRef} className="w-full flex items-center justify-center p-1 sm:p-2">
            <QRCardItem
                config={config}
                menuUrl={menuUrl}
                logoUrl={logoUrl}
