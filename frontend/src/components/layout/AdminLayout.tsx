...
const adminNavItems = [
    { to: '/admin', icon: LayoutDashboard, label: 'Overview', end: true, badge: false },
    { to: '/admin/restaurants', icon: Store, label: 'Restaurants & Access', badge: false },
    { to: '/admin/users', icon: Users, label: 'Users & Registrations', badge: false },
    { to: '/admin/activity', icon: FileText, label: 'Audit Log', badge: false },
    { to: '/admin/logs', icon: AlertOctagon, label: 'Logs & Diagnostics', badge: true },
];
...
export const AdminLayout: React.FC = () => {
    ...
    const { data: logMetrics } = useSystemLogMetrics(30_000);
        ...
        <div className="flex h-screen bg-slate-950 text-slate-100 overflow-hidden selection:bg-indigo-500 selection:text-white">
            ...
            <aside className="hidden lg:flex flex-col w-64 bg-slate-900 border-r border-slate-800 flex-shrink-0">
                ...
                <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                    {adminNavItems.map(({ to, icon: Icon, label, end, badge }) => (
                        <NavLink
                            key={to}
                            to={to}
                            end={end}
                            className={({ isActive }) =>
                                cn(
                                    'flex items-center justify-between px-3.5 py-2.5 text-sm font-semibold rounded-xl transition-all',
                                    isActive
                            ...
                            <nav className="flex-1 py-4 space-y-1">
                                {adminNavItems.map(({ to, icon: Icon, label, end, badge }) => (
                                    <NavLink
                                        ...
                                        className={({ isActive }) =>
                                            cn(
                                                'flex items-center justify-between px-3.5 py-2.5 text-sm font-semibold rounded-xl',
